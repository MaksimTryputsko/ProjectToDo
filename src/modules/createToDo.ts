...
export function createLi(
  ...
  li.innerHTML = `<input id="${id}" class="checkBoxToDo" type="checkbox" ${controlChecked(
    completed
  )}><label class="firstStyle ${controlChecked(
    completed
  )}" for="${id}">${title}</label><div id=${id} class="deleteImg"></div>`;
  ul?.prepend(li);
  controlCheckBox();
  controlImgDelete();
}
